...
const desktopMain = resolve(root, 'desktop', 'main.ts');
...
assert.equal(existsSync(desktopMain), true, 'desktop/main.ts must exist');
assert.equal(existsSync(viteConfig), true, 'vite.config.ts must exist');
...
assert.doesNotMatch(source, /SCHEDULED_COLLECT_RUN_ON_START:\s*process\.env\.SCHEDULED_COLLECT_RUN_ON_START\s*\|\|\s*'0'/, 'desktop must not force startup collection off');
