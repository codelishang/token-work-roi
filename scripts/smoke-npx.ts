...
try {
  ...
  const cliPath = join(runDir, 'node_modules', '.bin', process.platform === 'win32' ? 'token-work.cmd' : 'token-work');
...
function stopChild(child) {
  ...
  return new Promise<void>((resolveStop, rejectStop) => {
    const timer = setTimeout(() => {
      ...
      rejectStop(new Error('installed CLI did not exit within 5 seconds'));
    }, 5000);
    child.once('close', () => {
      clearTimeout(timer);
      resolveStop();
