...
function modelTier(model, pricingStatus = '') {
  ...
  if (name.startsWith('gpt-5.6-sol') || name.startsWith('gpt-5.5') || name.includes('claude-opus') || name.includes('gemini-2.5-pro-long-context')) return 'heavy';
  if (name === 'gpt-5.6-terra' || name === 'gpt-5.3-codex' || name.includes('claude-sonnet') || name.includes('gemini-2.5-pro') || /kimi-k2[.-][67]/.test(name)) return 'mid';
