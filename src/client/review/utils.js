...
function getPeriod(id, today = new Date(), rows = []) {
  ...
  if (rollingDays[id]) {
    const days = rollingDays[id];
    const start = new Date(t); start.setDate(t.getDate() - (days - 1));
    ...
    return {
      ...
      pretty: id === 'today' ? localDateStr(t) : `${localDateStr(start).slice(5)} – ${localDateStr(t).slice(5)}`,
      prev: (function () {
...
function getCustomPeriod(range = {}, today = new Date()) {
  ...
  let endDateTime = String(range.endDateTime || `${fallback}T23:59`);
  if (startDateTime > endDateTime) endDateTime = startDateTime;
  const start = startDateTime.slice(0, 10) || fallback;
  const end = endDateTime.slice(0, 10) || start;
  return {
    id: 'custom',
    label: '自定义',
    start,
    ...
    startDateTime,
    endDateTime,
    pretty: `${startDateTime.replace('T', ' ')} – ${endDateTime.replace('T', ' ')}`,
    prev: null
  };
}
  ...
  if (byTool.length >= 2 && byTool[0].costUSD > 0 && byTool[1].costUSD > 0) {
    ...
    if (aCostPerM > bCostPerM * 1.6) {
      ...
      insights.push({
        ...
        detail: [
          { k: `${a.key} $/百万 tk`, v: U.money(aCostPerM) },
          { k: `${b.key} $/百万 tk`, v: U.money(bCostPerM) },
          { k: '官方价差距', v: U.money(a.costUSD - b.costUSD) }
        ],
        narrative: `如果将 ${a.key} 中的部分工作迁移到 ${b.key}，理论上能省下约 ${U.money((a.costUSD - a.totalTokens / 1e6 * bCostPerM))}。但要权衡场景适配。`
      });
    }
  }
