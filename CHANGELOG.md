...
- 新增 WorkBuddy 本地采集器：解析 `~/.workbuddy/traces/` 中的结构化 trace 文件，提取 token 用量。
- Codex 首次扫描改为逐行读取历史 JSONL；后续定时采集继续只读取变化日志的尾部。
- 补全 GLM-5V-Turbo 的官方价格缓存和定时刷新解析。
- Hy3 改用腾讯 TokenHub 官方价格，并纳入定时价格刷新。
